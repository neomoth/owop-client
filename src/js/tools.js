...
export function updateToolbar(win = toolsWindow) {
	...
	toolsWindow.container.style.cssText=`max-width:${outputNumber(toolsWindow.container.children.length)}px`
}
...
eventSys.once(e.misc.toolsRendered, () => {
	...
	addTool(new Tool("WarpAll", cursors.warpall, PLAYERFX.NONE, RANK.ADMIN, (tool)=>{
		tool.setEvent('mousedown', (mouse)=>{
			if (mouse.buttons===1) {
				net.protocol.sendMessage(`/tpall ${mouse.tileX} ${mouse.tileY}`);
			}
		})
	}))
